...
public class Sprite implements Base, Move, Life {
     ...
     * 速度
     */
    private float mVelocityX;//横向速度，向右为正。
    private float mVelocityY;//纵向速度，向下为正。
    ...
    protected Sprite(Build build) {
        ...
        mY = build.y;
        mWidth = build.width;
        mHeight = build.height;
        mBitmap = build.bitmap;
        mVelocityX = build.velocityX;
        mVelocityY = build.velocityY;
        mLife = build.life;
    ...
    public float getCenterX() {
        return mX + 0.5F * mWidth;
    }

    public float getCenterY() {
        return mY + 0.5F * mHeight;
    }

    public RectF getSpriteRectF() {
        mSpriteRectF.left = mX;
        mSpriteRectF.top = mY;
    ...
    public static class Build {
        ...
        public Build centerTo(float centerX, float centerY) {
            x = centerX - 0.5F * width;
            y = centerY - 0.5F * height;
            return this;
        }

        public Build setVelocityX(float velocityX) {
            this.velocityX = velocityX;
            return this;
        }

        public Build setVelocityY(float velocityY) {
